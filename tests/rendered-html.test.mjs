...
test("server-renders the finished portfolio and research projects", async () => {
  ...
  assert.match(html, /İbrahim Damar — Edebiyat, Kültür ve Sanat/);
...
test("keeps the GitHub Pages version, motion system, and project media in sync", async () => {
  ...
  for (const text of [appPage, staticPage]) {
    ...
    assert.match(text, /İbrahim Damar — Edebiyat, Kültür ve Sanat/);
    ...
    assert.match(text, /AKA Endüstriyel — Akıllı Yedek Parça Kataloğu/);
    assert.match(text, /aka-endustriyel-demo\/#parca-ara/);
    assert.match(text, /Akademi360 — Kişisel Sınav Koçluğu/);
    assert.match(text, /akademi360-kocluk\/#giris/);
    assert.match(text, /Col Tur — Antalya Tur ve Rezervasyon Deneyimi/);
